...
module web() {
    union() {
        ...
        color("Green") translate([0, web_width, 0]) rotate([0, 0, -45]) cube([50, 7, 7]);

        translate([web_height + (key_radius / 2), key_radius - (key_radius / 2), 0]) rotate([0, 0, 135]) key_shape(7);
        color("Blue") translate([web_height + key_radius, key_radius, 0]) rotate([0, 0, 135]) cube([50, 7, 7]);

        translate([web_height + (key_radius / 2), web_width + (key_radius / 2), 0]) rotate([0, 0, -135]) key_shape(7);
        color("Red") translate([web_height, web_width + 5, 0]) rotate([0, 0, -135]) cube([50, 7, 7]);
    }
}